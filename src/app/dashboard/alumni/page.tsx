    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">{total} alumni in database</p>
        </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-zinc-800">
              {alumni.length === 0 ? (
                ...
                alumni.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className="group hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Link href={`/dashboard/alumni/${a.id}`} className="text-sm font-semibold text-gray-900 hover:text-amber-600 dark:text-zinc-100">
                              {a.name}
                            </Link>
                      ...
                      <td className="px-4 py-3">
                        <p className="text-sm font-medium text-gray-900 dark:text-zinc-100">{a.batch}</p>
                        <p className="text-xs text-gray-400">{a.branch}</p>
                      </td>
                      <td className="px-4 py-3">
                        {a.currentCompany ? (
                          <div>
                            <p className="text-sm text-gray-900 dark:text-zinc-100">{a.currentCompany}</p>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-500">
                        {a.assignedCoordinator?.name ?? <span className="text-gray-300">—</span>}
                      </td>
                    </tr>
                  );
                })
